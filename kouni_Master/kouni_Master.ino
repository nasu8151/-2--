...
void loop() {
  ...
  if (Serial.available()) {
    ...
    if (index1 > 0 && index2 > 0 && index3 > 0) {
      data0 = input.substring(0, index1).toInt();
      data1 = input.substring(index1 + 1, index2).toInt();
      data2 = input.substring(index2 + 1, index3).toInt();
      data3 = input.substring(index3 + 1).toInt();

      // 確認用に出力
      Serial.print("data0 = "); Serial.println(data0);
